...
class TaskCloud(AccessControlledModel):
    """
    Tasks are used to store job with its status and cluster description
    for a given user. Every task can only be ran by its owner.
    Once ran, a task can not evolve, so you can only clone or delete it.
    """
    ...
    def createTask(self, parent, name, description='', parentType='folder',
                    taskTemplate=None, clusterTemplate=None, creator=None):
        """
